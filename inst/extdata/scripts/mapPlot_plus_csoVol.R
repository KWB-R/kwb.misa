...
##### old files (with wrong precipitation data)
# path <- "Y:/AUFTRAEGE/_Auftraege_laufend/MISA3/Data-Work packages/AP1_Vorbereitung-Strategiebewertung/Schnittstelle/output/alt"
#
# df <- read.csv(file = file.path(path, "E10_2014_20211222_16-29_stats.csv"),
#                header = TRUE, sep = ";", dec = ".")
if(FALSE){
  path <- paste0(
  ...
  for(stat_file in stat_files){
    ...
    if(!is.null(dl_misa[[p_title]]$hours)){
      ...
      plot(x = 0, y = 0, xaxt = "n", yaxt = "n", type = "n",
      ...
      for(i in 1:length(ezg)){
       col <- colCircle[i]
        ...
        text(x = mean( ezg[[i]][,1]), y = mean(ezg[[i]][,2]), labels = names(ezg)[i])
      }
...
add_catchments <- function(catchmentList, col = "gray90"){
